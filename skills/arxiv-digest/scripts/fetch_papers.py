...
def process_papers(hf_papers, limit, filter_llm=False):
    """Process and score papers from HF API."""
    # LLM-related keywords for filtering
    llm_keywords = [
        "llm", "language model", "large language", "gpt", "bert", "transformer",
        "nlp", "natural language", "text generation", "chat", "dialogue",
        "instruction tuning", "rlhf", "fine-tuning", "lora", "adapter",
        "embedding", "tokenization", "attention", "generation", "prompt",
        "multimodal", "vision-language", "speech", "translation",
        "summarization", "question answering", "qa system", "reasoning"
    ]
    ...
    for i, item in enumerate(hf_papers):
        ...
        # Filter by LLM keywords if requested
        if filter_llm:
            text_to_check = " ".join([
                paper["title"].lower(),
                paper.get("summary", "").lower(),
                paper.get("ai_summary", "").lower(),
                " ".join(paper.get("keywords", [])).lower()
            ])
            if not any(kw in text_to_check for kw in llm_keywords):
                continue
...
def main():
    ...
    parser.add_argument("--filter-llm", action="store_true",
                       help="Filter papers to LLM-related topics only")
    ...
    papers = process_papers(hf_papers, args.limit, filter_llm=args.filter_llm)
